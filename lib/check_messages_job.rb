...
Capybara.app_host = "http://www.pathofexile.com"
Capybara.default_wait_time = 5

class CheckMessagesJob
  ...
  def self.perform
    ...
    page.fill_in 'Email', :with => "[removed]"
    page.fill_in 'Password', :with => "[removed]"
    ...
    page.all('table.pm-list tr').count.times do
      page.all('table.pm-list tr').each do |row|
        ...
        if status.has_text?('Unread')
          ...
          seller = Seller.find_or_create_by(account_name: seller.text)
          puts seller.inspect
          page.click_on('Reply')
