...
RESQUE_PROCESSORS = [
  #[queue_name, count]
  ["search_queue, account_queue", 2],
  ["index_queue, destroy_shop_queue, verify_queue", 14]
]
...
God.watch do |w|
  ...
  w.env = {"ROOT_URL"=>"http://www.poemarkets.com", "TERM_CHILD"=>1, "QUEUES"=>queue, "RESQUE_WORKER"=>'true', "RAILS_ENV"=>RAILS_ENV}
  w.start = "rake -f #{RAILS_ROOT}/Rakefile resque_environment resque:work"
  #w.pid_file = "#{RAILS_ROOT}/tmp/pids/resque-#{Digest::MD5.hexdigest(queue)}.pid"
